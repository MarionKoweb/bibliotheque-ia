...
function buildCard(item){
  ...
  /* le formulaire de personnalisation, replié tant qu’on ne clique pas */
  var perso=document.createElement("div");
  perso.className="kwb-perso"; perso.hidden=true;
  var lab=document.createElement("label"); lab.textContent="Ta demande";
  var dem=document.createElement("textarea");
  dem.placeholder="Par exemple : mets le titre « Rentrée des coachs », le prix à 497 € et mon lien https://monsite.systeme.io/commande";
  ...
  go.className="btn btn-primary kwb-go"; go.textContent="Ouvrir Claude avec ma demande";
  go.href="https://claude.ai/new"; go.target="_blank"; go.rel="noopener";
  var etat=document.createElement("span"); etat.className="kwb-etat";
  var ligne=document.createElement("div"); ligne.className="kwb-perso-actions";
  ligne.appendChild(go); ligne.appendChild(etat);
  perso.appendChild(lab); perso.appendChild(dem); perso.appendChild(info); perso.appendChild(ligne);

  ai.addEventListener("click",function(){
    perso.hidden=!perso.hidden;
    ai.textContent=perso.hidden?"Personnaliser":"Fermer";
    if(!perso.hidden) dem.focus();
  });
  go.addEventListener("click",function(e){
    var d=dem.value.trim();
    if(!d){ e.preventDefault(); etat.textContent="Écris d’abord ce que tu veux changer."; dem.focus(); return; }
    go.href=lienClaude(item,d);
    copieSecours(messageClaude(item,d));
    etat.textContent="Claude s’ouvre dans un nouvel onglet : clique sur Envoyer.";
  });

  var pre=document.createElement("pre");
  pre.className="code"; pre.hidden=true;

  card.appendChild(head); card.appendChild(stage); card.appendChild(foot); card.appendChild(perso); card.appendChild(pre);
